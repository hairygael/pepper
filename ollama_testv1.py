# -*- coding: utf-8 -*-
"""
Pepper + Ollama llama3.2 (UTF-8 safe + check clé API)
------------------------------------------------------
Fonctionnalités :
...
- Envoi du flux audio à Ollama (modèle llama3.2)
...
# ---------------- ENVOI À OLLAMA ----------------
def send_to_ollama(wav_bytes, tts):
    """
    Envoie le flux audio encodé à Ollama et renvoie le texte transcrit.
    ...
    try:
        ...
        # Construction du JSON attendu par l’API Ollama
        payload = {
            "model": "llama3.2",
            "messages": [
                {"role": "system",
...
def main():
    ...
    # Création d’un topic minimal pour activer sur le mot "bonjour"
    topic_content = u"""
    topic: ~hotword()
    language: frf
    u:(bonjour *) $1
    ...
    safe_print("[OK] Topic créé")

    # Prépare la capture micro
    recorder = AudioCapture(session, leds)

    # Fonction qui envoie le flux audio à Ollama et lit la réponse
    def handle_transcription():
        wav_bytes = recorder.stop()
        if wav_bytes:
            txt = send_to_ollama(wav_bytes, tts)
            if txt:
                safe_print("[OLLAMA]", txt)
    ...
    # Message d’accueil
    tts.say("Dis 'bonjour' pour m'activer.")
    app.run()
